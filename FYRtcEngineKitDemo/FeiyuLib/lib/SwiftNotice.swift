...
let RightTextFieldFont = UIFont.systemFont(ofSize: 13)
...
extension UIViewController {
    ...
    func noticeTop(_ text: String, autoClear: Bool = SwiftNotice.detaultIsAutoClear, autoClearTime: Int = 1) {
        SwiftNotice.noticeOnStatusBar(text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    // new apis from v3.3
    func noticeSuccess(_ text: String, autoClear: Bool = SwiftNotice.detaultIsAutoClear, autoClearTime: Int = SwiftNotice.defaultAutoClearTime) {
        SwiftNotice.showNoticeWithText(NoticeType.success, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeError(_ text: String, autoClear: Bool = SwiftNotice.detaultIsAutoClear, autoClearTime: Int = SwiftNotice.defaultAutoClearTime) {
        SwiftNotice.showNoticeWithText(NoticeType.error, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeInfo(_ text: String, autoClear: Bool = SwiftNotice.detaultIsAutoClear, autoClearTime: Int = SwiftNotice.defaultAutoClearTime) {
    ...
    func successNotice(_ text: String, autoClear: Bool = true) {
        SwiftNotice.showNoticeWithText(NoticeType.success, text: text, autoClear: autoClear, autoClearTime: SwiftNotice.defaultAutoClearTime)
    }

    func errorNotice(_ text: String, autoClear: Bool = true) {
        SwiftNotice.showNoticeWithText(NoticeType.error, text: text, autoClear: autoClear, autoClearTime: SwiftNotice.defaultAutoClearTime)
    }

    func infoNotice(_ text: String, autoClear: Bool = true) {
        SwiftNotice.showNoticeWithText(NoticeType.info, text: text, autoClear: autoClear, autoClearTime: SwiftNotice.defaultAutoClearTime)
    }
    
    func notice(_ text: String, type: NoticeType, autoClear: Bool, autoClearTime: Int = SwiftNotice.defaultAutoClearTime) {
    ...
    func rotateScreenPleaseWait(){
        SwiftNotice.rotateScreenWait()
    }
...
class SwiftNotice: NSObject {
    static var defaultAutoClearTime:Int = 3
    static var detaultIsAutoClear:Bool = true
    ...
    static func rotateScreenWait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        if imageNames.count > 0 {
            if imageNames.count > timerTimes {
                ...
                //                timer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(rawValue: UInt(0)), queue: DispatchQueue.global(qos: DispatchQoS.QoSClass.default))
                timer.scheduleRepeating(deadline: DispatchTime.init(uptimeNanoseconds: UInt64(timeInterval)*NSEC_PER_MSEC), interval: DispatchTimeInterval.seconds(1), leeway: DispatchTimeInterval.seconds(0))
        ...
        window.center = getCrossScreenRealCenter()//getRealCenter()
        // change orientation
        window.transform = CGAffineTransform(rotationAngle: CGFloat(degree * M_PI / 180))
        window.isHidden = false
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        let labelFont = RightTextFieldFont
        
        var labelFrame:CGRect = CGRect(x: 0, y: 60, width: frameWidth, height: 16)
        var frame = CGRect(x: 0, y: 0, width: frameWidth, height: 90)
        
        let realLabelSize = calcLabelTextSizeWithWidthLimit(text, font: labelFont, widthLimit: frameWidth)
        print("realLabelSize=\(realLabelSize)") //(78.0, 47.0)
        //let label = UILabel(frame: CGRectMake(0, 60, 90, 16))
        
        if realLabelSize.height > labelFrame.height {
            let moreHeight:CGFloat = realLabelSize.height - labelFrame.height
            print("moreHeight=\(moreHeight)") //31.0
            
            print("before labelFrame=\(labelFrame)") //(0.0, 60.0, 90.0, 16.0)
            labelFrame = CGRect(x: labelFrame.origin.x, y: labelFrame.origin.y, width: labelFrame.width, height: realLabelSize.height)
            print("after  labelFrame=\(labelFrame)") //(0.0, 60.0, 90.0, 47.0)
            
            print("before frame=\(frame)") //(0.0, 0.0, 90.0, 90.0)
            frame = CGRect(x: frame.origin.x, y: frame.origin.y, width: frame.width, height: frame.height + moreHeight)
            print("after  frame=\(frame)") //(0.0, 0.0, 90.0, 121.0)
        }
        ...
        //let label = UILabel(frame: CGRectMake(0, 60, frameWidth, realLabelSize.height))
        let label = UILabel()
    ...
    static func getCrossScreenRealCenter() -> CGPoint {
        if UIApplication.shared.statusBarOrientation.hashValue >= 3 {
            return CGPoint(x: rv!.center.x, y: rv!.center.y)
        } else {
            return rv!.center
        }
    }
